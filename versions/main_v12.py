...
def _market_orders(obs, me, priv, ctx, day, hour):
    ...
    if hour == 0:
        ...
        if not endgame:
            ...
            if room > 4:
                ...
                cow_worth = _remaining_value("COW", day, milk_px) > 400 * 2
                sheep_worth = _remaining_value("SHEEP", day, wool_px) > 500 * 2
                for _ in range(2):
                    cow_lag = ct / max(1, ctx["cow_target"])
                    sheep_lag = st / max(1, ctx["sheep_target"])
                    if cow_lag <= sheep_lag:
                        if cow_worth and money > 400 and ct < min(ctx["cow_target"], ctx["n_pastures"]):
                            orders.append(["BUY_ANIMAL", "COW", 1]); money -= 400; room -= 1; ct += 1
                            continue
                        if sheep_worth and money > 500 and st < min(ctx["sheep_target"], ctx["n_pastures"] - ct):
            ...
            for quad, (d0, d1), tiles in MELON_WAVES:
                ...
                if quad in unlocked and d0 <= day <= d1:
                    ...
                    need = int(min(max(0, tiles - have), 4, max(0, money - 200) // 80))
                    if need > 0:
    ...
    for item in PRODUCTS:
        ...
        if opp_ripe.get(item, 0) >= 6:
            score *= 1.5      # they are about to dump this channel
