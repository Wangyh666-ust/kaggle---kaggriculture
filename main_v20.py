...
def _order(tiles, anchor):
    """Nearest-neighbour chain from the shed-access tile: consecutive tiles in
    the returned order are adjacent, so watering a block is a single sweep.
    (Distance is inline: this runs at import time, before _dist is defined.)"""
    def d(a, b):
        return abs(a[0] - b[0]) + abs(a[1] - b[1])

    left, out, cur = list(tiles), [], anchor
    while left:
        nxt = min(left, key=lambda p: (d(cur, p), p[1], p[0]))
        left.remove(nxt)
        out.append(nxt)
        cur = nxt
    return out
...
_CENTER_SET = {(4, 4), (5, 4), (4, 5), (5, 5)}   # shed-access standing tiles
_NW_FREE = [(x, y) for y in range(5) for x in range(5)
            if (x, y) not in PASTURE_SPOTS_NW and (x, y) not in COOP_SPOTS
            and (x, y) not in _CENTER_SET]
_NE_FREE = [(x, y) for y in range(5) for x in range(5, 10)
            if (x, y) not in _CENTER_SET]
_SW_FREE = [(x, y) for y in range(5, 10) for x in range(5)
            if (x, y) not in PASTURE_SPOTS_SW and (x, y) not in _CENTER_SET]
NW_CROPS = _order(_NW_FREE, (4, 4))
NE_CROPS = _order(_NE_FREE, (5, 4))
SW_CROPS = _order(_SW_FREE, (4, 5))

# Fixed blocks: each crop owns its tiles so planting fills one contiguous
# region instead of scattering into whatever happens to be free (scattered
# crops cost the workers a walk for every single action).
MELON_BLOCK = {"NW": NW_CROPS[:6], "NE": NE_CROPS[:6]}
STRAWBERRY_BLOCK = NW_CROPS[6:] + NE_CROPS[6:22]
WHEAT_BLOCK = NE_CROPS[22:] + SW_CROPS
...
def _worker_tasks(day, ctx, seeds):
    ...
    if not endgame:
        ...
        # planting, gated by watering capacity. Each crop plants only inside
        # its own block, filling from the shed outward, so the field stays
        # contiguous and a worker sweeps a region instead of criss-crossing.
...
def agent(obs):
    ...
    ctx = {
        ...
        "unlocked_tiles": {(x, y) for y in range(BOARD) for x in range(BOARD)
