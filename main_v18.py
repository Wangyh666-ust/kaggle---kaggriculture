...
def _worker_tasks(day, ctx, seeds):
    ...
    if not endgame:
        ...
        if len(ctx["plants"]) < ctx["plant_capacity"]:
            ...
            # wheat is the lowest-value filler ($40/unit) — grow only what the
            # herd eats, and leave the rest of the land for strawberries ($214)
            wheat_need = len(ctx["animals"]) + 6
            wheat_now = sum(1 for (_, _, t) in ctx["plants"] if t["crop"] == "WHEAT")
            if day <= 24 and seeds.get("WHEAT", 0) > 0 and wheat_now < wheat_need:
                for (x, y) in ctx["empty"]:
                    if (x, y) in PASTURE_SPOTS_NW or (x, y) in PASTURE_SPOTS_SW:
                        continue
...
def agent(obs):
    ...
    strawberry_target = max(14, min(44, 16 + 6 * straw_shops))
    if straw_price < 110:
        strawberry_target = min(strawberry_target, 24)
    if straw_price < 90:
        strawberry_target = 0
